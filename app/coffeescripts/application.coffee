$ ->
	...
	$(".slider > *").each ->
		$(this).css
			backgroundImage: "url('/images/#{$(this).attr "class"}.jpg')"
	...
	.click ->
		linha = $(this).parent().attr("data-linha")
		window.location = "/produtos/#{linha}"
	...
	$(".midias-sociais-buttons img").hover -> 
		...
		$(this).attr "src", "/images/hover-#{midia}-icon.png"

	, ->	
		midia = $(this).attr "data-midia"
		$(this).attr "src", "/images/#{midia}-icon.png"

	# Tooltip
	$.fn.tooltip = (texto) ->
		$(this).hover ->
			$("<div class='tooltip'><img src='/images/tip.png' height='20'>#{texto}</div>").appendTo("body").hide().css 
